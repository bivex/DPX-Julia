"""Interactive HTML Observability HUD Formatter for Julia."""
...
CATEGORY_COLORS: dict[PatternCategory, dict[str, str]] = {
    PatternCategory.JULIA_IDIOMATIC: {
        "text": "#a855f7",
        "bg": "#581c8744",
        "border": "#9333ea",
        "accent": "#c084fc",
        "name": "Julia Idiomatic",
    },
    PatternCategory.MULTIPLE_DISPATCH: {
        "text": "#38bdf8",
        "bg": "#0c4a6e44",
        "border": "#0284c7",
        "accent": "#0ea5e9",
    ...
    PatternCategory.CONCURRENCY_PARALLELISM: {
        "text": "#34d399",
        "bg": "#064e3b44",
        "border": "#059669",
        "accent": "#10b981",
    ...
    PatternCategory.SCIENTIFIC_PERFORMANCE: {
        "text": "#22d3ee",
        "bg": "#164e6344",
        "border": "#0891b2",
        "accent": "#06b6d4",
        "name": "Scientific Performance",
    },
...
class HtmlReportFormatter(ReportFormatterPort):
    """Renders standalone, responsive HTML dashboard for Julia DetectionReport with AI Prompt Generator."""
    ...
    def format(self, report: DetectionReport, verbose: bool = False) -> str:
        ...
        for idx, det in enumerate(report.detections, 1):
            ...
            finding_prompt = (
                f"Refactor and optimize the following Julia architectural pattern / performance finding:\n"
                ...
                f"Please provide an idiomatic Julia 1.9 - 1.11+ high-performance refactoring leveraging Multiple Dispatch, Holy Traits, Parametric Structs, and Zero-Allocation Views."
            )
...
<head>
    ...
    <title>Pattern Scanner Report - DPX-Julia Architecture HUD - {html.escape(report.project_path or "Codebase")}</title>
    <style>
        :root {{
            --bg: #0b0f17;
            --card-bg: #131b26;
        ...
        * {{ box-sizing: border-box; margin: 0; padding: 0; font-family: -apple-system, BlinkMacSystemFont, "Segoe UI", Roboto, monospace, sans-serif; }}
        ...
        .btn-ai-context {{
            background: linear-gradient(135deg, #9558b2 0%, #389826 50%, #cb3c33 100%);
            color: #ffffff;
            border: none;
            padding: 10px 18px;
            border-radius: 8px;
            ...
            align-items: center;
            gap: 8px;
            box-shadow: 0 4px 14px rgba(149, 88, 178, 0.35);
            transition: transform 0.15s, box-shadow 0.15s;
        }}
        .btn-ai-context:hover {{
            transform: translateY(-2px);
            box-shadow: 0 6px 20px rgba(149, 88, 178, 0.5);
        }}
        .btn-view-prompt {{
        ...
        .search-bar:focus {{ border-color: var(--julia-purple); }}
        ...
        .pattern-card:hover {{ border-color: #9558b288; box-shadow: 0 4px 20px rgba(0,0,0,0.3); }}
        ...
        .btn-copy-card-ai {{
            ...
            color: #c084fc;
            border: 1px solid rgba(149, 88, 178, 0.35);
            padding: 4px 10px;
            border-radius: 6px;
            font-size: 11.5px;
            font-weight: 600;
        ...
        .btn-copy-card-ai:hover {{
            background: rgba(149, 88, 178, 0.3);
            color: #ffffff;
            border-color: #9558b2;
        }}
...
<body>
    <div class="container">
        <header>
            <div>
                <h1>🎯 DPX-Julia Architecture & Performance Observability HUD</h1>
                <div class="subtitle">Multiple Dispatch, Holy Traits, Metaprogramming, Tasks & High-Performance Engine • Target: <code>{html.escape(report.project_path or "Target Repository")}</code></div>
            </div>
        ...
        <input type="text" id="searchInput" class="search-bar" placeholder="🔎 Instant search by pattern name, category, target type, or rule (e.g. traits, dispatch, task, views, struct)...">
    ...
    <div id="promptModal" class="modal-overlay" onclick="closeModalOnOverlay(event)">
        <div class="modal-box">
            <div class="modal-header">
                <div class="modal-title">🤖 AI Architect Context Prompt (Julia)</div>
                <button class="modal-close" onclick="closePromptModal()">&times;</button>
            </div>
            <div class="modal-body">
                <p style="font-size: 13px; color: #94a3b8; margin-bottom: 12px;">Structured XML/Markdown context for LLMs (Claude, GPT-4, Gemini) including Multiple Dispatch, Holy Traits, and performance hazard analysis.</p>
    ...
    <script>
        ...
        filterBtns.forEach(btn => {{
            btn.addEventListener('click', () => {{
                ...
                btn.style.outline = '2px solid #9558b2';
                filterCards();
            }});
        }});
    </script>
