"""Token-Efficient LLM / AI Prompt Context Formatter for Julia."""
...
class LlmReportFormatter:
    ...
    def format_scan_report(self, report: DetectionReport) -> str:
        ...
        lines: list[str] = [
            '<codebase_architecture_analysis language="julia">',
        ...
        if patterns:
            lines.append("    <julia_design_patterns_and_multiple_dispatch>")
            for d in patterns:
        ...
        if hazards:
            lines.append("    <performance_hazards_and_type_instabilities>")
            for v in hazards:
