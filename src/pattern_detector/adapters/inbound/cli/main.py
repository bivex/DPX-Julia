"""Typer and Rich CLI interface for DPX-Julia."""
...
app = typer.Typer(
    name="dpx-julia",
    help="🎯 Modern Julia Static Analyzer: Multiple Dispatch, Holy Traits, Metaprogramming, Tasks, GoF & High-Performance Architectural Pattern Detector.",
...
def scan(
    path: Annotated[
        str,
        typer.Argument(
            help="Path to a Julia source file (.jl) or directory containing a Julia package/project.",
        ),
    ] = ".",
    ...
    pattern: Annotated[
        ...
        typer.Option(
            "--pattern",
            "-p",
            help="Filter for specific pattern type(s) (e.g. 'holy_traits_dispatch', 'multiple_dispatch_polymorphism', 'type_instability_non_concrete_field').",
        ),
    ] = None,
    ...
    exclude: Annotated[
        ...
        typer.Option(
            "--exclude",
            "-e",
            help="Directory name(s) to exclude from scanning (e.g. -e deps -e docs).",
        ),
    ] = None,
    verbose: Annotated[
        bool,
    ...
    """Scan a Julia codebase for Multiple Dispatch, Holy Traits, Concurrency, GoF patterns, and type instability hazards."""
...
@app.command()
def rules() -> None:
    """List all available Julia detection rules and performance checks."""
    table = Table(
        title="🎯 DPX-Julia: Supported Pattern Rules & Quality Checks",
...
def info(
    pattern_name: Annotated[
        str,
        typer.Argument(
            help="Pattern type or name to query (e.g. 'holy_traits_dispatch', 'multiple_dispatch_polymorphism').",
        ),
    ],
) -> None:
    """Display deep architectural details, rationale, and advice for a specific Julia pattern."""
    ...
    panel = Panel.fit(
        ...
        f"[bold white]Julia Target:[/bold white] {pdef.julia_version}\n\n"
        f"[bold white]Description:[/bold white]\n{pdef.description}\n\n"
        f"[bold white]Architecture Recommendation:[/bold white]\n{pdef.recommendation or 'Adheres to standard Julia high-performance idiom.'}",
        title=f"🔍 Julia Architecture Encyclopedia: {pdef.name}",
