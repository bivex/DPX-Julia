"""Outbound driven ports for DPX-Julia."""

from __future__ import annotations

from typing import Protocol
from pattern_detector.domain.code_model import CodeModel, JuliaFile
from pattern_detector.domain.detection import DetectionReport


class SourceProviderPort(Protocol):
    """Port for discovering and loading Julia source files."""
...
class ParserPort(Protocol):
    """Port for parsing Julia source text into CodeModel."""

    def parse_file(self, file_path: str, content: str) -> JuliaFile:
        """Parse a single Julia file into JuliaFile model."""
        ...

    def parse_codebase(self, files: list[tuple[str, str]], target_path: str = "") -> CodeModel:
        """Parse multiple Julia files into an aggregated CodeModel."""
        ...
