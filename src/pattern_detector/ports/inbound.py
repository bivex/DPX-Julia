"""Inbound driving ports for DPX-Julia."""
...
@dataclass
class ScanOptions:
    """Configuration options for a Julia scanning session."""

    min_confidence: float = 0.0
    enabled_patterns: list[str] = field(default_factory=list)
    file_extensions: list[str] = field(default_factory=lambda: [".jl"])
...
class ScannerPort(Protocol):
    """Inbound port for scanning a target directory or Julia source file."""
